document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayAlarms() {
    ...
    alarms.forEach((alarm, index) => {
      ...
      deleteIcon.className = "fa-regular fa-trash"; // Font Awesome trash icon class
  ...
  function stopAlarm() {
    if (alarmIntervalId) {
      clearInterval(alarmIntervalId);
      alarmIntervalId = null; // Reset the interval ID
    }
  }

  // Function to set an alarm with sound and notification
  function setAlarm(medicineName, alarmTime) {
    ...
    if (timeToAlarm > 0) {
      ...
      // Set a timeout to start the alarm
      setTimeout(() => {
